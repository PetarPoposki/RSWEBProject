...
namespace RSWEBProject.Areas.Identity.Pages.Account
{
    public class LoginModel : PageModel
    {
        ...
        public LoginModel(SignInManager<RSWEBProjectUser> signInManager, ILogger<LoginModel> logger, RSWEBProjectContext context)
        {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    RSWEBProjectUser user = await _context.Users.FirstOrDefaultAsync(m => m.Email == Input.Email);
                    ...
                    if (role == "Client")
                    {
                        ...
                        var pom = await _context.Client.Where(x => x.RSWEBProjectUserId == user.Id).FirstOrDefaultAsync();
                        HttpContext.Session.SetString("UserLoggedIn", pom.Id.ToString());

                        string pomosen = "/Restaurants/ViewOrders/" + pom.Id;

                        return LocalRedirect(pomosen);
                    }
                    else if (role == "Delivery Man")
                    {
                        var pom = await _context.DeliveryMan.Where(x => x.RSWEBProjectUserId == user.Id).FirstOrDefaultAsync();
                        
                        
                        HttpContext.Session.SetString("UserLoggedIn", pom.Id.ToString());

                        string pomosen = "/Restaurants/ViewRestaurants/" + pom.Id;
                        return LocalRedirect(pomosen);
                    }
